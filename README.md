...
给**别的领域的人**用的编程技能集：设计师、写作者、老师、做生意的、研究别的学科的。你不写代码，也不用学；你带进来的是创意、想法和你那行的判断，代码是 AI 的活。

设计前提是**把你的想法做成能用的东西，同时让你看懂、并且始终由你掌舵**：活全部 AI 干完，每个专业名词当场翻译成人话，每做一步都告诉你它干了什么。

改编自 [Matt Pocock 的 skills](https://github.com/mattpocock/skills)（MIT）。原版是给熟练工程师用的强指令，这套把它改成了不写代码的人也能全程跟得上、并且始终说得上话的版本。
...
| 谁 | 干什么 |
| --- | --- |
| AI | 配环境、写代码、写测试、修报错、技术上的取舍、提交存档、翻译术语 |
| 你 | 三件事：**只有你能做的**（注册、授权、付钱、亲眼验收）、**只有你有的**（内容、文案、审美、你那行的规矩）、**方向上的决定**（做给谁、先做哪个、值不值得花这个钱） |

四条贯穿始终的原则：活 AI 干完创意和方向问你、术语当场翻译成人话、一次只推进一件事、**每跑一步跟一句解释**（不攒着汇报，不只贴命令输出）。

**纯技术的取舍不会拿来烦你。** 用哪个框架、数据存成什么格式、CSS 怎么组织，你没有判断依据，AI 自己定完交待一句就行。要问你的一律是你能用自己的话回答的：「你平时在哪写东西？」你能答，「Markdown 存 Git 还是接 CMS？」不用你答。
